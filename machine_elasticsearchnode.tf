...
resource "aws_instance" "elasticsearchnode-machine" {
  ...
  connection {
    ...
    host        = "${aws_instance.elasticsearchnode-machine[each.value].public_ip}"
  }
  ...
  tags = {
    ...
    region = "us-east-1"
    env = "AAAAA"
    AnsibleRole = "elasticsearchnode"
    ClusterRole = "none"
  }
}


resource "aws_route53_record" "elasticsearchnode-machine-private-record" {
  for_each = "${toset(var.elasticsearchnode_machine_names)}"
  zone_id  = "${data.aws_route53_zone.dns_private_zone.zone_id}"
  name     = "${each.value}.${data.aws_route53_zone.dns_private_zone.name}"
  type     = "A"
  ttl      = "300"
  records  = ["${aws_instance.elasticsearchnode-machine[each.value].private_ip}"]
}


resource "aws_route53_record" "elasticsearchnode-machine-reverse-record" {
  for_each = "${toset(var.elasticsearchnode_machine_names)}"
  zone_id = "${data.aws_route53_zone.dns_reverse_zone.zone_id}"
  name    = "${element(split(".", aws_instance.elasticsearchnode-machine[each.value].private_ip),3)}.${element(split(".", aws_instance.elasticsearchnode-machine[each.value].private_ip),2)}.${data.aws_route53_zone.dns_reverse_zone.name}"
...
resource "aws_route53_record" "elasticsearchnode-machine-public-record" {
  for_each = "${toset(var.elasticsearchnode_machine_names)}"
  zone_id  = "${data.aws_route53_zone.dns_public_zone.zone_id}"
  name     = "${each.value}.AAAAA.${data.aws_route53_zone.dns_public_zone.name}"
  type     = "A"
  ttl      = "300"
  records  = ["${aws_eip.elasticsearchnode-machine-eip[each.value].public_ip}"]
}
*/
