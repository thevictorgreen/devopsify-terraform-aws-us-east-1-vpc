...
resource "aws_instance" "hadoop-machine" {
  for_each      = "${toset(var.hadoop_machine_names)}"
  ami           = "${var.amis["ubuntu_18_04"]}"
  instance_type = "${var.instance_type["micro"]}"

  key_name      = "${var.keypairs["kp_1"]}"
  subnet_id     = "${var.subnets[ var.hadoop_machine_subnets[ each.value ] ]}"
  ...
  connection {
    ...
    host        = "${aws_instance.hadoop-machine[each.value].public_ip}"
  }
...
resource "aws_route53_record" "hadoop-machine-private-record" {
  for_each = "${toset(var.hadoop_machine_names)}"
  zone_id  = "${data.aws_route53_zone.dns_private_zone.zone_id}"
  name     = "${each.value}.${data.aws_route53_zone.dns_private_zone.name}"
  type     = "A"
  ttl      = "300"
  records  = ["${aws_instance.hadoop-machine[each.value].private_ip}"]
}


resource "aws_route53_record" "hadoop-machine-reverse-record" {
  for_each = "${toset(var.hadoop_machine_names)}"
  zone_id = "${data.aws_route53_zone.dns_reverse_zone.zone_id}"
  name    = "${element(split(".", aws_instance.hadoop-machine[each.value].private_ip),3)}.${element(split(".", aws_instance.hadoop-machine[each.value].private_ip),2)}.${data.aws_route53_zone.dns_reverse_zone.name}"
...
resource "aws_eip" "hadoop-machine-eip" {
  for_each = "${toset(var.hadoop_machine_names)}"
  instance = "${aws_instance.hadoop-machine[each.value].id}"
  vpc      = true
}


resource "aws_route53_record" "hadoop-machine-public-record" {
  for_each = "${toset(var.hadoop_machine_names)}"
  zone_id  = "${data.aws_route53_zone.dns_public_zone.zone_id}"
  name     = "${each.value}.AAAAA.${data.aws_route53_zone.dns_public_zone.name}"
  type     = "A"
  ttl      = "300"
  records  = ["${aws_eip.hadoop-machine-eip[each.value].public_ip}"]
}
*/
